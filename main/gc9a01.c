...
void lcd_send_byte(uint8_t Data) { lcd_data(&Data, 1); }

void delay_ms(uint32_t Delay_ms) { vTaskDelay(Delay_ms / portTICK_PERIOD_MS); }

uint16_t GC9A01_GetWidth() { return GC9A01_Width; }

uint16_t GC9A01_GetHeight() { return GC9A01_Height; }
...
static void ColumnSet(uint16_t ColumnStart, uint16_t ColumnEnd) {
  ...
  if (ColumnEnd > GC9A01_Width) return;
...
static void RowSet(uint16_t RowStart, uint16_t RowEnd) {
  ...
  if (RowEnd > GC9A01_Height) return;
...
static void MemAccessModeSet(uint8_t Rotation, uint8_t VertMirror,
  ...
  if (VertMirror) Ret = MADCTL_ML;
  if (HorizMirror) Ret = MADCTL_MH;

  if (IsBGR) Ret |= MADCTL_BGR;
...
void GC9A01_FillRect(int16_t x, int16_t y, int16_t w, int16_t h,
  ...
  for (uint32_t i = 0; i < (h * w); i++) GC9A01_RamWrite(&color, 1);
}
